function [delta_for_plot,w_for_plot,v_for_plot] =Runge_Kutta3(P,numG,Pe,H,D,TG,KG,Td,Tdd,Tq,xd,xdd,xddd,xl,id,Kd,Kq,vd,vq,KA,TA,...
    ...
    max,Yprime,Rg,Glabel,removeTime,delta_for_plot,w_for_plot,v_for_plot,dt,now_step,endTime,...
...
for n = 1:max
  %<<<<<<<<<<<<<< earth fault >>>>>>>>>>>>>>>
  if n == round(removeTime/dt) && removeAccidentYesNo == 1% remove accident n*dt
...
[vd_atk vq_atk] = RK_vdq(delta(n,:)+dt_k(k,:).*delta_k(k,:),Yg,...
  ...
  if mod(n,10000) == 0
    str = ['EFT ',num2str(removeTime),' n ',num2str(n)];
    disp(str)
    toc
    tic
  end
